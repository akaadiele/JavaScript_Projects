...
const apiLink = `https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=${apiKey}&page=1`;  // Constructing the API URL for fetching popular movies from TMDB, including the API key and sorting by popularity.
        ...
        .then(function (data) {
            data.results.forEach(element => {
                ...
                if (!element.poster_path || !element.title) {
                    ...
                    divCol.innerHTML = `    
                        <div class="card text-center movie-card" style="width: 18rem;">
                    ...
                    // imgElement.setAttribute("src", `${imgPath + element.poster_path}`);                    
                    // pElement.innerHTML = `[${element.release_date.split("-")[0]}]  <strong> | </strong> <a href="movie.html?id=${element.id}&title=${element.title}" class="movie-link">View Reviews</a>`;
